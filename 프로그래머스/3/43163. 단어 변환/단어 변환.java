...
class Solution{
    ...
    public int solution(String begin, String target, String[] words){
        ...
        while(!q.isEmpty()){
            Node cur = q.poll() ; 
            if(cur.word.equals(target)){ // 만약 queue에서 뺀 단어가 target이면
                answer = cur.steps ; //  변환 완료 --> 현재까지의 steps 반환 
                break ; 
            } // if 
            
            // queue에서 뺀 단어가 중간 단어면 
            for(int i = 0 ; i < n ; i++){ // 다른 단어들로의 변환 가능여부 확인 
                if(!visited[i] && isConvertable(cur.word, words[i])){ // 방문하지 않았고, 해당 노드로 변환 가능하면
