"""
핵심 Point
: 시간 복잡도가 기본 O(N^3) == O(10^6)이기 때문에 이동 좌표/방향 계산을 O(1)에 끝내야한다. 

이동 칸 수가 클 때
1. 한번에 계산
2. lookup 테이블 활용 --> 그냥 이어붙여서 참조하는 방식 ** (아이디어 얻어가자)

=> 교훈 : 시간 복잡도를 필수로 계산하고, 규칙성 파악이 필요한 문제인지 반드시 판단 !
    ㄴ 시뮬 유형 파악 : [직관적 구현] / [규칙성 발견] 으로 구분

# 상어 수가 많지 않기 때문에 개체 형태로 모아서 정렬해도 성능 괜찮!!
...
for j in range(C): # O(N)
    # 1. 낚시왕 이동
    for i in range(len(shk)) :  # O(N^2) : 상어 최대 마리 수
        # 상어를 한마리씩 검사하면서
    ...
    # 2. 상어 이동
    for k in range(len(shk)):  # O(N^2) : 상어 최대 마리 수
        # ************ 좌표, 방향 계산 : O(1) *************
        if shk[k][3] >= 3:  # 우, 좌 방향
    ...
    shk.sort(key = lambda x: (x[1], x[0], -x[4])) # O(N^2)
    # 열, 행 오름차순, 크기 내림차순
    # 파이썬은 정렬 성능이 좋음
    for i in range(len(shk)-1, 0, -1) :  # O(N^2): 가장 끝에서 1번까지
